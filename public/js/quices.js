...
$( document ).on("click","#agregar",function(e) {
    e.preventDefault();
    var quiz = new Object();
    quiz.titulo=$("#titulo").val();
    quiz.descripcion=$("#descripcion").val();
    quiz.fecha_inicio=$("#fechaInicio").val();
    quiz.fecha_fin=$("#fechaFin").val();
    quiz.curso=$("#listaCursos").val();
    console.log(quiz);
    var ajax = __ajax2("https://noanlearning.space/profesores/quices/quiz",JSON.stringify(quiz));
     ajax.done(function(response){ 
        var estudiante=JSON.parse(response);
        if (estudiante.estado==1) {
            $("#agregar").html("Quiz Agregado");
            setTimeout(function(){
                $("#agregar").html("Agregar Quiz");
            },2000)
        }
     });
 });
